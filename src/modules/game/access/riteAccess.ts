...
export const RITE_GAME_CONFIG: Record<MasonicRite, RiteGameConfig> = {
  reaa: {
    ...
    boardSize: 33,
    masonic: true,
    description: 'Tablero masónico del Rito Escocés Antiguo y Aceptado.',
    boardNote: '33 casillas como identidad visual del sistema de 33 grados del REAA.',
  },
  york: {
    rite: 'york',
    boardSize: 12,
    masonic: true,
    description: 'Tablero inspirado en la progresión habitual del Rito de York.',
    boardNote: '12 etapas como configuración inicial de juego. La estructura puede variar por jurisdicción y se podrá editar por administración.',
  },
  frances: {
    rite: 'frances',
    boardSize: 7,
    masonic: true,
    description: 'Tablero inspirado en los tres grados simbólicos y cuatro Órdenes de Sabiduría del Rito Francés.',
    boardNote: '7 etapas iniciáticas como convención de juego; el quinto orden histórico es administrativo/conservatorio y no se cuenta como grado de la escala.',
  },
  'nacional-mexicano': {
    rite: 'nacional-mexicano',
    boardSize: 9,
    masonic: true,
    description: 'Tablero del Rito Nacional Mexicano.',
    boardNote: '9 casillas de progresión principal, en referencia a sus nueve grados históricos.',
  },
  libre: {
    rite: 'libre',
    boardSize: 30,
    masonic: false,
    description: 'Maratón de cultura general abierto a cualquier jugador.',
    boardNote: 'Este modo NO ES MASÓN. Usa preguntas de historia, ciencia, geografía, cultura y conocimiento general.',
  },
  otro: {
    rite: 'otro',
    boardSize: 30,
    masonic: true,
    description: 'Sala para un banco masónico personalizado.',
    boardNote: 'La cantidad de casillas puede personalizarse cuando administración defina el rito y su estructura.',
  },
}
...
const q = (
  ...
  correctAnswer: number,
  explanation?: string,
): RiteExamQuestion => ({ rite, id, text, options, correctAnswer, explanation })

/**
 * Exámenes de acceso del juego. Deliberadamente usan conocimiento público y
 * NO incluyen palabras, signos, toques ni datos de reconocimiento reservados.
 * Un administrador puede sustituirlos por un cuestionario autorizado de su jurisdicción.
 */
export const RITE_EXAMS: Record<Exclude<MasonicRite, 'libre' | 'otro'>, RiteExamQuestion[]> = {
  reaa: [
...
export function levelRank(level: RoomLevel): number {
  if (level === 'general') return 0
  return DEGREE_ORDER.indexOf(level) + 1
}

export function certificationCovers(certification: RiteCertification | undefined, target: RoomLevel): boolean {
  if (!certification) return false
  if (target === 'general') return true
  return certification.rite !== 'libre' && levelRank(certification.level) >= levelRank(target)
}

export function requiresAccessExam(
  preferredRite: MasonicRite | null,
  profileDegree: MasonicDegree | null,
  roomRite: MasonicRite,
  roomLevel: RoomLevel,
  certification?: RiteCertification,
): boolean {
  if (roomRite === 'libre') return false
