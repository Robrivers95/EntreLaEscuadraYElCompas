<template>
  <div class="game-board" :class="`rite-${rite}`">
        ...
        >
          ...
          <div v-if="getPlayersAtPosition(node.position).length" class="tree-players">
            <span
              ...
              :title="player.name"
            >
              <i class="mini-head" :style="{ backgroundColor: player.color }"></i>
              <i class="mini-body" :style="{ backgroundColor: player.color }"></i>
              <b>{{ player.initials }}</b>
            </span>
          </div>
        </div>
    ...
    <div v-else-if="rite === 'reaa'" class="reaa-shell">
      <div class="mobile-board-note">↔ Desliza el tablero para recorrer las 33 casillas</div>
      <div class="reaa-scroll">
        <div class="reaa-board">
...
const props = withDefaults(defineProps<Props>(), { rite: 'reaa' })

defineEmits<{ (e: 'cell-click', cell: BoardCellType): void }>()

const boardCells = computed(() => {
  const cells: BoardCellType[] = []
  const cols = Math.ceil(Math.sqrt(props.boardSize))
  for (let i = 0; i < props.boardSize; i += 1) {
    cells.push({
      id: i,
      x: i % cols,
      y: Math.floor(i / cols),
...
const getPlayersAtPosition = (position: number) => props.players
  .map((player, index) => ({
    index,
    name: player.name,
  ...
  .map(({ index, name, initials: shortName, color }) => ({ index, name, initials: shortName, color }))
</script>

<style scoped>
.game-board{display:flex;justify-content:center;align-items:center;padding:12px;width:100%;box-sizing:border-box;min-width:0}.board-container{width:100%;max-width:690px}.board-frame{padding:17px;border:2px solid #c9a84c;border-radius:18px;background:radial-gradient(circle at 50% 35%,rgba(201,168,76,.10),transparent 45%),linear-gradient(145deg,rgba(74,35,10,.88),rgba(20,9,4,.97));box-shadow:inset 0 0 0 5px rgba(73,42,18,.7),inset 0 0 40px rgba(0,0,0,.42),0 25px 55px rgba(0,0,0,.42)}.board-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(82px,1fr));gap:9px;padding:8px;border-radius:12px;background-image:linear-gradient(rgba(255,255,255,.018) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.018) 1px,transparent 1px);background-size:22px 22px}
.reaa-shell{width:100%;min-width:0}.reaa-scroll{width:100%;overflow:visible;box-sizing:border-box;touch-action:pan-x pan-y pinch-zoom}.mobile-board-note{display:none;text-align:center;padding:6px 8px;color:rgba(238,224,194,.5);font-size:9px;text-transform:uppercase;letter-spacing:.07em}.reaa-board{width:100%;max-width:760px;aspect-ratio:1;margin:0 auto}.reaa-frame{position:relative;width:100%;height:100%;border:3px solid #d6b75f;border-radius:22px;background:radial-gradient(circle at center,rgba(21,57,76,.95),rgba(5,13,22,.98) 60%),linear-gradient(145deg,#4c270e,#170b04);box-shadow:inset 0 0 0 7px rgba(79,44,18,.8),inset 0 0 55px rgba(0,0,0,.5),0 26px 65px rgba(0,0,0,.45);overflow:hidden}.perimeter-side{position:absolute;display:grid;gap:5px;z-index:2}.side-bottom{left:1.5%;right:1.5%;bottom:1.5%;grid-template-columns:repeat(9,1fr)}.side-top{left:12.5%;right:12.5%;top:1.5%;grid-template-columns:repeat(8,1fr)}.side-left{left:1.5%;top:12.5%;bottom:12.5%;width:10%;grid-template-rows:repeat(8,1fr)}.side-right{right:1.5%;top:12.5%;bottom:12.5%;width:10%;grid-template-rows:repeat(8,1fr)}.side-left :deep(.board-cell),.side-right :deep(.board-cell){min-height:0;height:100%;aspect-ratio:auto;padding:4px}.side-left :deep(.cell-category),.side-right :deep(.cell-category){font-size:6.5px}.side-top :deep(.board-cell),.side-bottom :deep(.board-cell){min-height:0;aspect-ratio:1}.reaa-center{position:absolute;inset:17%;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;border:1px solid rgba(214,183,95,.28);border-radius:18px;background:radial-gradient(circle,rgba(214,183,95,.1),transparent 62%)}.reaa-center span{font:700 clamp(13px,2vw,22px) Georgia,serif;letter-spacing:.08em;color:#ead89b;text-transform:uppercase}.reaa-center strong{margin-top:7px;font:900 clamp(24px,4vw,48px) Georgia,serif;color:#d6b75f}.reaa-center small{color:rgba(238,224,194,.55);letter-spacing:.16em;text-transform:uppercase}.reaa-seal{font:900 clamp(30px,6vw,72px) Georgia,serif;color:rgba(214,183,95,.78);margin-bottom:13px}.reaa-seal b{color:#f4df99}
.kabbalah-shell{width:100%;max-width:660px;padding:18px 20px 34px;border:2px solid #c7ab60;border-radius:24px;background:radial-gradient(circle at 50% 35%,rgba(58,123,118,.18),transparent 36%),radial-gradient(circle at 50% 5%,rgba(218,189,101,.13),transparent 28%),linear-gradient(180deg,#07141d,#03080d);box-shadow:inset 0 0 45px rgba(0,0,0,.5),0 26px 62px rgba(0,0,0,.44)}.kabbalah-title{text-align:center;display:flex;flex-direction:column;gap:3px;margin-bottom:8px}.kabbalah-title span{color:#c7ab60;text-transform:uppercase;letter-spacing:.2em;font-size:11px}.kabbalah-title strong{color:#f0dda1;font:800 29px Georgia,serif}.kabbalah-title small{color:rgba(238,224,194,.48);letter-spacing:.08em}.tree-canvas{position:relative;height:min(780px,115vw);min-height:570px;margin:0 auto;max-width:540px}.tree-lines{position:absolute;inset:2% 0 0;width:100%;height:96%;overflow:visible}.tree-lines line{stroke:rgba(202,174,92,.38);stroke-width:.55;filter:drop-shadow(0 0 2px rgba(201,168,76,.2))}.sefirah{position:absolute;width:88px;height:88px;transform:translate(-50%,-50%);display:grid;place-items:center;z-index:2}.sefirah-ring{position:absolute;inset:0;border-radius:50%;border:1px solid rgba(221,195,113,.62);background:radial-gradient(circle,rgba(206,177,87,.18),rgba(17,54,62,.62) 58%,rgba(2,8,12,.96) 72%);box-shadow:0 0 18px rgba(207,176,82,.16),inset 0 0 14px rgba(255,255,255,.04)}.sefirah.occupied .sefirah-ring{border-color:#f1d77d;box-shadow:0 0 28px rgba(224,194,102,.32),inset 0 0 16px rgba(224,194,102,.12)}.sefirah-core{position:relative;z-index:2;text-align:center;display:flex;flex-direction:column}.sefirah-core span{font-size:7px;letter-spacing:.08em;color:rgba(235,217,159,.46)}.sefirah-core strong{font:700 13px Georgia,serif;color:#f1dda0}.tree-players{position:absolute;z-index:6;display:flex;gap:2px;bottom:-17px;left:50%;transform:translateX(-50%);pointer-events:none}.tree-person{position:relative;width:24px;height:34px;display:block;filter:drop-shadow(0 3px 3px rgba(0,0,0,.45))}.mini-head{position:absolute;top:0;left:7px;width:11px;height:11px;border-radius:50%;border:1px solid rgba(255,255,255,.75);box-shadow:inset 2px 2px 3px rgba(255,255,255,.22),inset -2px -2px 3px rgba(0,0,0,.24)}.mini-body{position:absolute;left:4px;top:9px;width:17px;height:20px;border-radius:6px 6px 5px 5px;border:1px solid rgba(255,255,255,.58);box-shadow:inset 3px 2px 4px rgba(255,255,255,.18),inset -3px -2px 4px rgba(0,0,0,.25)}.tree-person b{position:absolute;left:0;right:0;top:15px;text-align:center;color:rgba(8,14,18,.78);font:900 5px system-ui;z-index:3}.tree-start{position:absolute;left:50%;bottom:-19px;transform:translateX(-50%);min-width:110px;text-align:center;padding:7px 10px;border:1px dashed rgba(201,168,76,.35);border-radius:999px;color:rgba(238,224,194,.55);font-size:9px;letter-spacing:.12em;text-transform:uppercase}.start-players{bottom:-37px}
@media(max-width:720px){.game-board{padding:3px}.board-frame{padding:8px}.board-grid{grid-template-columns:repeat(auto-fit,minmax(68px,1fr));gap:5px;padding:3px}.mobile-board-note{display:block}.reaa-scroll{overflow:auto;padding:0 0 9px;-webkit-overflow-scrolling:touch;scrollbar-color:rgba(214,183,95,.4) transparent;overscroll-behavior:contain}.reaa-board{width:620px;min-width:620px;max-width:none}.reaa-frame{border-radius:14px}.perimeter-side{gap:4px}.side-left :deep(.cell-category),.side-right :deep(.cell-category),.side-top :deep(.cell-category),.side-bottom :deep(.cell-category){font-size:6.5px}.side-left :deep(.characters),.side-right :deep(.characters){transform:scale(.78);transform-origin:bottom center}.reaa-center{inset:18%}.kabbalah-shell{padding:14px 6px 36px}.tree-canvas{min-height:520px}.sefirah{width:70px;height:70px}.sefirah-core strong{font-size:11px}.tree-person{transform:scale(.9);margin:0 -2px}}
</style>
