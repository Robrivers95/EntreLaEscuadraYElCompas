  ...
  <div class="admin-panel">
    <div class="panel-header">
      <div>
        ...
        <p>Administra rito, grado y categoría de cada pregunta del tablero.</p>
      </div>
      <button class="btn-seed" :disabled="seeding" @click="importStarterQuestions">
        {{ seeding ? 'Agregando…' : '＋ Importar bancos iniciales' }}
      </button>
    </div>
    ...
    <div class="questions-list">
      <div class="list-controls">
        ...
        <select v-model="filterRite" class="filter-select">
          <option value="">Todos los ritos</option>
          <option v-for="rite in MASONIC_RITES" :key="rite.value" :value="rite.value">{{ rite.shortLabel }}</option>
        </select>
      ...
      <div v-else-if="filteredQuestions.length === 0" class="no-questions">
        ...
        <p>Puedes crear una arriba o importar el banco REAA inicial.</p>
      </div>

      <div v-else class="questions-grid">
        <article v-for="question in filteredQuestions" :key="question.id" class="question-card">
          <div class="question-header">
            <span class="rite-badge">{{ RITE_SHORT_LABELS[getQuestionRite(question)] }}</span>
            <span class="difficulty-badge" :class="question.difficulty">{{ DEGREE_LABELS[question.difficulty] }}</span>
            <span class="category-badge">{{ question.category }}</span>
          </div>
          <p class="question-text">{{ question.text }}</p>
          <div class="question-options">
            <div v-for="(option, index) in question.options" :key="index" class="option" :class="{ correct: index === question.correctAnswer }">
          ...
          <p v-if="question.source" class="source">Fuente: {{ question.source }}</p>
...
import { DEGREE_LABELS, getQuestionRite, MASONIC_CATEGORIES, MASONIC_RITES, normalizeAnswer, RITE_SHORT_LABELS } from '@/modules/questions/questionRules'
...
const filterDifficulty = ref<MasonicDegree | ''>('')
...
const filteredQuestions = computed(() => {
  ...
  return questionsStore.questions.filter((question) => {
    const matchRite = !filterRite.value || getQuestionRite(question) === filterRite.value
    ...
    const haystack = normalizeAnswer([question.text, ...question.options, question.directAnswer ?? '', ...(question.acceptedDirectAnswers ?? []), question.source ?? ''].join(' '))
    return matchRite && matchCategory && matchDifficulty && (!search || haystack.includes(search))
  })
})
...
const duplicateQuestion = async (question: Question) => {
  ...
  const text = `${copy.text} (copia)`
  try {
    const id = await questionsService.addQuestion({ ...copy, text })
    questionsStore.addQuestion({ ...copy, text, id })
  } catch (err) {
...
const importStarterQuestions = async () => {
  seeding.value = true
  try {
    const existing = new Set(questionsStore.questions.map((question) => `${getQuestionRite(question)}|${normalizeAnswer(question.text)}`))
    const missing = STARTER_QUESTIONS.filter((question) => !existing.has(`${getQuestionRite(question)}|${normalizeAnswer(question.text)}`))
    for (const question of missing) await questionsService.addQuestion(toQuestionInput(question))
    await reloadQuestions()
    alert(missing.length ? `Se agregaron ${missing.length} preguntas. Incluye el banco REAA por grado y categoría.` : 'Los bancos iniciales ya estaban cargados.')
  } catch (err) {
    console.error('Error importing starter questions:', err)
    alert('No se pudieron cargar los bancos iniciales en Firebase.')
...
.panel-header p { margin:6px 0 0; color:rgba(240,230,200,.7); }
...
.list-controls { display:grid; grid-template-columns:minmax(230px,1fr) auto auto auto auto; gap:10px; margin-bottom:12px; }
...
.summary-row { display:flex; gap:16px; flex-wrap:wrap; padding:10px 0 18px; color:rgba(240,230,200,.72); font-size:13px; } .summary-row strong { color:#c9a84c; }
.status-message,.no-questions { text-align:center; padding:35px 20px; color:#c9a84c; border:1px dashed #8b6914; border-radius:10px; }.status-message.error{color:#ff8b8b;border-color:#8f4343}
.questions-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(340px,1fr)); gap:18px; }
.question-card { background:linear-gradient(145deg,rgba(201,168,76,.07),rgba(26,10,0,.75)); border:1px solid #8b6914; border-radius:12px; padding:16px; transition:.25s ease; }.question-card:hover{transform:translateY(-3px);border-color:#c9a84c;box-shadow:0 12px 30px rgba(0,0,0,.25)}
.question-header{display:flex;gap:7px;flex-wrap:wrap;margin-bottom:12px}.rite-badge,.difficulty-badge,.category-badge{padding:5px 9px;border-radius:999px;font-size:11px;font-weight:800;text-transform:uppercase}.rite-badge{background:#c9a84c;color:#1a0a00}.category-badge{background:rgba(139,105,20,.3);color:#f0e6c8;border:1px solid #8b6914}.difficulty-badge{color:white}.difficulty-badge.aprendiz{background:rgba(76,175,80,.72)}.difficulty-badge.compañero{background:rgba(230,165,20,.8)}.difficulty-badge.maestro{background:rgba(190,65,58,.82)}
.question-text{color:#f0e6c8;font-weight:650;line-height:1.45}.question-options{background:rgba(0,0,0,.2);border-left:3px solid #c9a84c;padding:9px 11px;border-radius:5px}.option{color:rgba(240,230,200,.82);padding:4px 0;font-size:13px}.option.correct{color:#8be19b;font-weight:700}
.direct-answer-preview{display:flex;gap:7px;margin-top:10px;font-size:12px;color:rgba(240,230,200,.65)}.direct-answer-preview strong{color:#c9a84c}.explanation,.source{color:rgba(240,230,200,.62);font-size:12px}.source{padding-top:8px;border-top:1px solid rgba(201,168,76,.12)}
...
@media(max-width:900px){.panel-header{align-items:stretch;flex-direction:column}.list-controls{grid-template-columns:1fr}}
</style>
