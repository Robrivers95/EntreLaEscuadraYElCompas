  ...
  <main class="duel-view">
    ...
    <template v-else>
      <section class="create-panel">
        <div><span class="kicker">Nuevo desafío</span><h2>Encuentra un rival</h2></div>
...
const setup = reactive<{ opponentUid: string; rite: MasonicRite; level: RoomLevel }>({ opponentUid: '__random__', rite: 'reaa', level: authStore.masonicDegree ?? 'aprendiz' })
...
const chooseOpponent = (): DuelPlayerOption | null => {
  if (players.value.length === 0) return null
  if (setup.opponentUid !== '__random__') return players.value.find((player) => player.uid === setup.opponentUid) ?? null
  return players.value[Math.floor(Math.random() * players.value.length)] ?? null
}
...
const startChallenge = async () => {
  ...
  if (!opponent) { message.value = 'No hay otro usuario disponible para asignar como rival.'; return }
  setup.opponentUid = opponent.uid
  ...
  message.value = `Rival asignado: ${opponent.name}.`
}
...
const answer = async (optionIndex: number) => {
  ...
  if (sessionRole.value === 'challenger') {
    ...
    message.value = `Reto enviado a ${opponent.name}: ${score} correctas y ${10 - score} incorrectas.`
    setup.opponentUid = '__random__'
  } else if (activeDuel.value) {
    await duelService.finishDuel(activeDuel.value, score)
    const wrong = 10 - score
    message.value = score > activeDuel.value.challengerScore
      ? `Ganaste: ${score} correctas, ${wrong} incorrectas. Tu rival hizo ${activeDuel.value.challengerScore}/10.`
      : score < activeDuel.value.challengerScore
        ? `Perdiste: ${score} correctas, ${wrong} incorrectas. Tu rival hizo ${activeDuel.value.challengerScore}/10.`
        : `Empate: ${score} correctas y ${wrong} incorrectas cada uno.`
  }
